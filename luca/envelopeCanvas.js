// Make ar global so it can be accessed by save/load scripts
window.ar = {
...
function drawEnv() {
  ...
  // points in px
  const P0 = { x: 0, y: h };
  const P1 = { x: window.ar.xA * w, y: 0 };
  const P2 = { x: window.ar.xR * w, y: 0 };
  const P3 = { x: w, y: h };

  // envelope polyline
  envCtx.strokeStyle = "#f0f0f0ff";
  envCtx.lineWidth = 3;
  envCtx.beginPath();
  ...
  readout.textContent = `xA=${window.ar.xA.toFixed(3) ?? 0}  xR=${window.ar.xR.toFixed(3) ?? 0}`;
}

function drawHandle(label, x, y) {
  envCtx.fillStyle = "#fff";
  envCtx.strokeStyle = "#111111ff";
  envCtx.lineWidth = 2;
  envCtx.beginPath();
  envCtx.arc(x, y, 7, 0, Math.PI * 2);
  envCtx.fill();
  envCtx.stroke();
  envCtx.fillStyle = "#efbc05ff";
  envCtx.font = "20px ui-monospace, monospace";
  envCtx.fillText(label, x + 10, y + 14);
}

function pick(mx, my) {
  const w = envCanvas.width;
  const A = { x: window.ar.xA * w, y: 20 };
  const R = { x: window.ar.xR * w, y: 20 };
...
function toNormX(mx, who) {
  switch (who) {
    case "A":
      return Math.min(window.ar.xR, Math.max(0.05, mx / envCanvas.width));
    case "R":
      return Math.min(0.98, Math.max(window.ar.xA, mx / envCanvas.width));
    default:
      console.log("Errore");
      return;
  }
...
envCanvas.addEventListener("pointermove", (e) => {
  ...
  if (dragging === "A") window.ar.xA = x;
  if (dragging === "R") window.ar.xR = x;
